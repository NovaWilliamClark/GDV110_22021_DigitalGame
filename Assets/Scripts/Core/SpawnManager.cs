...
public class SpawnManager : MonoBehaviour
{
    ...
    public void DoSpawn()
    {
        ...
        {
            if (dataSo.sceneName == currentSpawnData.LevelDataAtSpawn.sceneName)
            {
                currentLevelData[currentLevelData.IndexOf(dataSo)] = currentSpawnData.LevelDataAtSpawn;
                continue;
            }
            if (dataSo.createdAt > currentSpawnData.LevelDataAtSpawn.createdAt)
            {
                Debug.LogFormat("Removed {0} level data", dataSo.sceneName);
